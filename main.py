...
if __name__ == "__main__":
    ...
    df_plot = df.drop(['event', 'is_signal', 'true_pi0_pair'], axis=1) # df ready for plot
    columns_df_plot = df_plot.columns
    ...
    print(f"photon features: {columns_df_plot}")
    #print(f"photon 4-mom (E, px, py, pz): ({df[]})")

    ## Plot 3 photon 4-momentum
    plot_hist(df_plot, 3, 100, "Photon 4-momentum") # data, column list, number of rows to plot, number of bins
    ...
    #('m_gg', 'E_asym'),             # Check if mass correlates with asymmetry
    #('opening_angle', 'E_asym'),    # Opening angle vs energy asymmetry
    #('cos_theta', 'opening_angle'),  # These are directly related
    #('E1', 'E2'),                    # Photon energies (should be anti-correlated for π⁰)
    feature_columns = ['m_gg', 'opening_angle', 'cos_theta', 'E_asym', 'E1', 'E2']
    ...
    # E_asym           strong negative (-0.6+)     pi0 decays are symmetric
    ...
    for _, evt in result_events.iterrows():
        ...
        if evt.is_signal:
            ...
            if best_pair == evt.true_pi0_pair:
                ...
                score_signal.append(score)
                status = "✓"
            else:
                score_background.append(score)
                status = "✗"
        else:
            score_background.append(score)
            status = "BG"
    ...
    ## Mass and score distribution analysis
    fig, axes = plt.subplots(1, 2, figsize=(10, 8))
    fig.suptitle("Mass and Score", fontsize=16, y=1.02)

    # Create data structure
    print(rf'{len(score_signal)},     {len(score_background)}')
    score_signal_df = pd.DataFrame({
        'score': score_signal,
        'type': 'signal_correct'
    })
    score_background_df = pd.DataFrame({
        'score': score_background,
        'type': 'background_or_wrong'
    })
    all_score_df = pd.concat([score_signal_df, score_background_df], ignore_index=True) # Combing data frame
    m_matched = [m for m, match in zip(candidate_masses, true_matches) if match]
    m_not_matched = [m for m, match in zip(candidate_masses, true_matches) if not match]
    print(all_score_df.describe())
    print(all_score_df)

    #print(m_matched)
    titles = [rf'$\pi^{0}$ Candidate Mass Distribution', 'Score']
    y_labels = ['Events', 'Events']
    x_labels = ['Invariant Mass (GeV)', 'Score']
    
    
    for i in range(2):
        if (i == 0): # mass distributions
            n, bin_edges, patches = axes[i].hist([m_matched, m_not_matched],
                                                bins=30, 
                                                alpha=0.5, 
            ...
            axes[i].axvline(x=0.135, color='black', linestyle='--', label='True pi0 mass')
        else:
            n, bin_edges, patches = axes[i].hist([score_signal, score_background],
                                                bins=100, 
                                                alpha=0.5, 
    ...
    plt.tight_layout()
    plt.savefig('./plots/mass_score.png')
    #plt.show()

 
    r'''
    plt.figure(figsize=(10, 5))
    plt.hist([m for m, match in zip(candidate_masses, true_matches) if match], bins=30, alpha=0.5, label='Correctly identified', color='green')
    plt.hist([m for m, match in zip(candidate_masses, true_matches) if not match], bins=30, alpha=0.5, label='Wrong identified', color='red')
